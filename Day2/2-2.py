...
def validate_password(passwordList:list):
    acceptablePasswords = []
    for policy in passwordList:
        firstPos = int(policy.split("-")[0]) - 1 # Account for policy not having index 0
        secondPos = int(policy.split("-")[1][0:2]) - 1 # Account for policy not having index 0
        character = str(policy.split(" ")[1][0])
        password = str(policy.split(" ")[2])
        if (password[firstPos] == character) != (password[secondPos] == character): # Python version of XOR
            acceptablePasswords.append(policy)
    print(len(acceptablePasswords))
...
finalList = validate_password(puzzleInput)